package org.example.filemonitoringapi.watcher;
...
public class FileWatcherService {
    ...
    public void watchFileChanges(String filePath, WatchService watchService) {
        while (!Thread.currentThread().isInterrupted()) {
            ...
            try {
                ...
                log.info("Wątek monitorujący zmiany plików został przerwany.");
                return;
            } catch (ClosedWatchServiceException e) {
                log.info("Serwis WatchService został zamknięty.");
                return;
            }
            ...
            for (WatchEvent<?> event : key.pollEvents()) {
                ...
                try {
                    ...
                    log.error("Błąd podczas przetwarzania zmiany pliku: " + filePath, e);
                }
            }

            boolean valid = key.reset();
            if (!valid) {
                log.info("Klucz WatchService stał się nieważny.");
                break;
            }
        }
    }
    ...
    private void handleFileModification(String filePath) {
        try {
            ...
            log.error("Błąd podczas obsługi modyfikacji pliku: " + filePath, e);
        }
    }
    ...
    public void sendPendingNotifications() {
        ...
        changesByFilePath.forEach((filePath, changes) -> {
            ...
            activeSubscriptions.forEach(subscription -> {
                ...
                emailSender.sendDetails(buildEmailData(subscription.getEmail(), "Zmiana w pliku: " + filePath, emailBody));

                changes.forEach(change -> {
                    fileChangeRepository.delete(change);
                });
    ...
    public EmailDetails buildEmailData(String to, String subject, String content) {
